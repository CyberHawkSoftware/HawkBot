module.exports = function utility(bot, info)
{
  ...
  //Loads a feather if there is one by the name
  ...
  utilities.isServerCommandForm = function(message, serverID)
  {
    return new Promise((resolve, reject) =>
    {
      info.db.findServer(serverID).then((serverInfo) =>
      {
        if(message.startsWith(serverInfo.prefix))
        {
          //console.log('true');
          resolve({'isCommand': true, 'prefix': serverInfo.prefix});
        }
        else
        {
          //console.log('false');
